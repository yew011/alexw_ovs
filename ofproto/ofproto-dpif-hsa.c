...
static void hsa_calculate_list(struct hsa_list *, uint8_t table_id,
                               struct ds *, int indent);
                       ...
                       uint8_t cur_table_id, struct ds *out, int indent)
{
    ...
    list_insert(&ret->hs_list, &hs->list_node);
    ...
    OFPACT_FOR_EACH (a, ofpacts, ofpacts_len) {
        ...
        case OFPACT_RESUBMIT: {
            ...
            /* Executes 'resubmit' action for each 'hs'. */
            ret = list_hsa_calculate_list(ret, table_id, out, indent++);
            break;
        }
...
hsa_init(struct ofproto *ofproto, ofp_port_t ofp_port, struct ds *out)
{
    ...
    hsa_result = xzalloc(sizeof *hsa_result);
    list_init(&hsa_result->hs_list);
}
...
{
    struct hsa_list *list = xzalloc(sizeof *list);
    ...
    LIST_FOR_EACH(rule, node, rules) {
        ...
        if (hsa_rule_check_match(hs, rule)) {
            ...
            if (debug_enabled) {
                ...
                hsa_match_print(out, &clone->match_hs, indent);
            }

            /* Applies the actions. */
            list = hsa_rule_apply_actions(clone, rule, table_id);
...
/* Given list of header spaces 'hsa_list', do 'hsa_calculate' for each
 * header space in the list.  Returns a list of new header spaces.
 *
 * Function will take ownership of 'hsa_list'.  */
static struct hsa_list *
hsa_calculate_list(struct hsa_list *hsa_list, uint8_t table_id, struct ds *out,
                   int indent)
{
    struct header_space *hs, *next;
    struct hsa_list *ret = hsa_list_create();

    LIST_FOR_EACH_SAFE (hs, next, list_node, &hsa_list->hs_list) {
        struct hsa_list *list = hsa_list_create();

        list = hsa_calculate(hs, table_id, out, indent);
    ...
    /* Destroys 'hsa_list', since alreay finishes using. */
    hsa_list_destroy(hsa_list);
                              ...
                              const char *argv[], void *aux OVS_UNUSED)
{
    ...
    list = hsa_calculate(hs, TABLE_DEFAULT, &out, INDENT_DEFAULT);
