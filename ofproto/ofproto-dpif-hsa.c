...
struct header_space {
    ...
    struct match match_constraint;  /* Contraints from previously flows. */
...
hsa_hsbt_apply_match(struct hsbt_list *hsbt_list, struct hsa_rule *rule)
{
    ...
    LIST_FOR_EACH (iter, list_node, &old_hsbt_list->list) {
        struct hsbt_list *diff = hsbt_diff(iter, hsbt_flow);
...
hsa_init(struct ofproto *ofproto, ofp_port_t ofp_port, struct ds *out)
{
    ...
    hs_start = xzalloc(sizeof *hs_start);
                              ...
                              const char *argv[], void *aux OVS_UNUSED)
{
    ...
    /* Print output. */
    ds_put_cstr(&out, "\n\n\nOUTPUT\n======\n");
    ds_put_format(&out, "%-16s   Input Header Space\n", "port_no");
    LIST_FOR_EACH (hs, list_node, &hsa_result->hs_list) {
...
static void
ofproto_dpif_unixctl_hsa_trace(struct unixctl_conn *conn, int argc OVS_UNUSED,
                               const char *argv[] OVS_UNUSED,
                               void *aux OVS_UNUSED)
{
    struct ds out = DS_EMPTY_INITIALIZER;

    /* Calls hsa_calculate(), enables the debug printout. */
...
hsa_unixctl_init(void)
{
    ...
    unixctl_command_register("hsa/trace", "bridge OpenFlow", 2, 2,
                             ofproto_dpif_unixctl_hsa_trace, NULL);
}

/* Public functions. */
