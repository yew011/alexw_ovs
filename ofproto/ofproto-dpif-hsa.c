...
struct header_space {
    ...
    struct list flow_contrain[__FLOW_ATTR_MAX]; /* Flow contrains, one for */
                                                /* flow attribute. */
};

/* Rule used for head space analysis. */
struct hsa_rule {
    struct list node;               /* In owning table's 'rules'. */
    int prio;                       /* Priority. */
...
struct hsa_table {
    struct list rules;              /* Contains 'struct hsa_rule's. */
...
static void hsa_init(struct ofproto *, ofp_port_t ofp_port);
...
hsa_rule_apply_output_action__(struct header_space *hs, ofp_port_t port)
{
    ...
    case OFPP_CONTROLLER:
        ...
        break;
    default:
        if (port != hs->match.flow.in_port.ofp_port) {
                       ...
                       uint8_t cur_table_id)
{
    ...
    OFPACT_FOR_EACH (a, ofpacts, ofpacts_len) {
        ...
        case OFPACT_SET_ETH_DST:
            memset(&hs_wc->masks.dl_dst, 0xff, sizeof wc->masks.hs_dl_dst);
...
static void
hs_init_(struct header_space *hs, ofp_port_t in_port)
{
    ...
    hs->match_flow.flow.in_port.ofp_port = ofp_port;
...
static void
hsa_init(struct ofproto *ofproto, ofp_port_t ofp_port)
{
...
hsa_finish(void)
{
    ...
    for (i = 0; i < n_hsa_tables; i++) {
        struct list *rules = &hsa_tables[i].rules;
...
static void
hsa_calculate(struct header_space *hs, uint8_t table_id)
{
    struct hsa_table *table = &hsa_tables[table_id];
    struct list *rules = &table->rules;
    struct hsa_rule *rule;
                              ...
                              const char *argv[], void *aux OVS_UNUSED)
{
    ...
    if (debug_enabled) {
        for (i = 0; i < n_hsa_tables; i++) {
            ...
            LIST_FOR_EACH(rule, node, rules) {
                ds_put_format(&ds, "table_id=%"PRIuSIZE", ", i);
                match_format(&rule->match, &ds, rule->prio);
                ds_put_cstr(&ds, ",actions=");
                ofpacts_format(rule->ofpacts, rule->ofpacts_len, &ds);
                ds_put_cstr(&ds, "\n");
            }
        }
    }
    ...
    hsa_calculate(hs, TABLE_DEFAULT);
    /* Cleans up. */
    hsa_finish();
}
